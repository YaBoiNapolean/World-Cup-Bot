...
# 📂 CHOOSE STORAGE PATH (Railway volumes use /data by default)
DATA_PATH = "/data/settings.json"
server_settings = {}

def load_settings():
    """Loads server configurations from the persistent volume."""
...
def save_settings():
    """Saves server configurations directly to the persistent volume."""
    try:
        # Create directory if it doesn't exist (useful for local testing)
...
    
    # Load data from volume storage immediately upon waking up
    load_settings()
...
        save_settings()
        
        await interaction.response.send_message(f"✅ **Timezone updated to:** `{tz_name}`", choices=None)
